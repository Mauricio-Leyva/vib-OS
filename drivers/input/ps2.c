...
void ps2_poll(void) {
  /* ps2_poll runs both from the main loop and from IRQ context. Without
   * mutual exclusion the IRQ can preempt the main loop in the middle of a
   * mouse packet, leading to torn packets and duplicate key dispatches
   * (visible as repeated letters when typing fast). Disable interrupts for
  ...
  for (int i = 0; i < 64; i++) {
    ...
    if (status & PS2_STATUS_MOUSE_DATA) {
      ...
      if (mouse_packet_index == 1 && !(data & 0x08)) {
        printk(KERN_DEBUG "PS2 MOUSE: Dropped byte 0x%02x (bit 3 not set)\n", data);
        mouse_packet_index = 0;
        continue;
      }
      
      if (mouse_packet_index >= 3) {
        uint8_t f = mouse_packet[0];
        /* PS/2 mouse delta is a 9-bit signed value: the low 8 bits come from
         * packet[1]/packet[2], and the sign bit lives in packet[0]: bit 4 for
         * X, bit 5 for Y. Using (int8_t) only sign-extends the low byte and
         * gets wrong-direction jumps on packets where the sign bit differs.
         * Drop packets where the overflow bits (bits 6/7) are set — the
         * device gave up encoding a too-large delta. */
        if (f & 0xC0) {
          mouse_packet_index = 0;
          continue;
        }
        int rx = (int)mouse_packet[1] - ((f << 4) & 0x100);
        int ry = (int)mouse_packet[2] - ((f << 3) & 0x100);
        mouse_packet_index = 0;

        /* Update position */
        ps2_mouse_x += rx * mouse_scale;
        ps2_mouse_y -= ry * mouse_scale;
        ...
        /* Buttons */
        ps2_mouse_buttons = 0;
        if (f & 0x01) ps2_mouse_buttons |= 1;
        if (f & 0x02) ps2_mouse_buttons |= 2;
        if (f & 0x04) ps2_mouse_buttons |= 4;

        if (rx != 0 || ry != 0 || ps2_mouse_buttons != 0) {
          printk(KERN_DEBUG "PS2 MOUSE: raw(%d,%d) pos(%d,%d) btn(%d)\n", 
                 (int)rx, (int)ry, ps2_mouse_x, ps2_mouse_y, ps2_mouse_buttons);
        }
      }
    } else {
      /* ============ KEYBOARD DATA ============ */
      ...
      printk(KERN_DEBUG "PS2 KBD: scancode 0x%02x\n", scancode);
...
int ps2_init(void) {
  printk(KERN_INFO "PS2: Initializing PS/2 controller...\n");
  ...
  printk(KERN_INFO "PS2: Initialization complete\n");
  return 0;
}
